...
모듈 구조, 런타임 경계, 씬 생명주기를 다룬다. 게임 규칙은 [01-game-design.md](./01-game-design.md), 에셋 규칙은 [03-content-and-assets.md](./03-content-and-assets.md)를 본다.
...
│   ├── grid.ts             격자 방향·통행 규칙
│   ├── fieldmaps.ts        야외 필드 맵 정의
│   ├── goblin-fortress.ts  던전 맵 정의
│   ├── towns.ts            마을 레지스트리와 이동 규칙
│   ├── entrances.ts        맵 입구 비주얼
│   ├── fpview.ts           1인칭 시점 렌더링
│   ├── hud.ts              탐험·전투 HUD
│   ├── battle-fx.ts        전투 타격 이펙트
│   ├── tiles.ts            타일 시트 등록과 슬라이스
│   ├── monsters.ts         몬스터 아이콘 로딩·폴백 드로잉
│   └── portraits.ts        초상화 로딩
...
**정의는 `game/defs/`, 규칙은 `game/core/`와 `game/state.ts`, 표현은 `game/scenes/`와 `game/ui/`에 둔다.** 정적 정의와 순수 규칙에는 PixiJS 표현 로직을 넣지 않는다. 씬과 UI는 규칙의 결과를 화면에 표현하고 사용자 입력을 명령으로 변환한다.
...
마을별 데이터와 공통 검증을 씬에서 분리한다. `types.ts`(타입), `crossvale.ts`·`evermore.ts`(마을 데이터), `compile.ts`(검증), `content.ts`(대화), `facilities.ts`(시설 처리기 연결), `navigation.ts`(마을 내 이동), `presentation.ts`(표현 보조), `world-state.ts`(월드 상태)로 구성한다. `compileTown()`은 잘못된 시설 좌표, 배치 중복, 접근 불가능한 통행 칸을 마을 진입 시 즉시 발견한다.
...
런타임 에셋은 `public/assets/`에서만 로드하며 `assets-source/`는 브라우저에서 직접 참조하지 않는다.
...
디렉터리 분류, 파일명 규칙과 등록 절차는 [03-content-and-assets.md](./03-content-and-assets.md)를 따른다.
...
구조 변경 후에는 [전체 검증](./README.md#실행과-검증)을 실행한다.
