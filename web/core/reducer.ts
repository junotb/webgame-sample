...
const MENACE_LABELS: Record<MenaceId, string> = {
  fatigue: "피로",
  scrutiny: "주목",
  unrest: "동요",
};
/** 표기 순서 = 언어 3종(듣기·말하기·쓰기) → 술법 2종. 패널이 이 순서로 읽는다 */
...
export function growthNotes(prev: GameState, next: GameState): string[] {
  ...
  for (const skill of Object.keys(SKILL_LABELS) as SkillId[]) {
    if (next.self.skills[skill] > prev.self.skills[skill]) {
      notes.push(
        `「${SKILL_LABELS[skill]}」 ${next.self.skills[skill]}등급이 되었다.`,
      );
    }
  }
  for (const stat of Object.keys(STAT_LABELS) as StatId[]) {
    if (next.self.stats[stat] > prev.self.stats[stat]) {
      notes.push(
        `${STAT_LABELS[stat]}이(가) ${next.self.stats[stat]}에 이르렀다.`,
      );
    }
  }
  ...
  if (next.self.memory > prev.self.memory) {
    notes.push(
      `기억이 남았다 (${next.self.memory}/7). 서류함의 옛 문서를 다시 열 수 있다.`,
    );
  }
  return notes;
}
...
export function cappedMenaces(prev: GameState, next: GameState): MenaceId[] {
  return (Object.keys(MENACE_LABELS) as MenaceId[]).filter(
    (m) =>
      prev.world.menace[m] < MENACE_CAP && next.world.menace[m] >= MENACE_CAP,
  );
}
...
function afterFieldPhase(
  ...
  return content.storylets.some((s) => evalConditions(state, s.requirements))
    ? "event"
    : "closing";
}
...
function assertPhase(
  state: GameState,
  expected: GameState["world"]["phase"],
  actionType: string,
): void {
  if (state.world.phase !== expected) {
    throw new Error(
      `${actionType}은 ${expected} 단계에서만 가능 (현재: ${state.world.phase})`,
    );
  }
}
  ...
  switch (action.type) {
    ...
    case "RESOLVE_MINIGAME": {
      ...
      if (state.world.shiftLeft < MINIGAME_TIME_COST) {
        throw new Error(
          `근무 시간 부족: 잔여 ${state.world.shiftLeft}, 필요 ${MINIGAME_TIME_COST}`,
        );
      }
      const next = structuredClone(state);
      ...
      if (next.world.shiftLeft <= 0)
        next.world.phase = afterFieldPhase(next, content);
      // 결과 반영 산문 — 성적 3변형에서 현재 상태로 선택 (인터랙션 순서: 미니게임 → 산문)
      const prose = order.resultProse
        ? selectProse(next, order.resultProse[action.result])
        : [];
      const log = [...prose];
      log.push(...growthNotes(state, next));
      return { state: next, log, notices: cappedMenaces(state, next) };
    ...
    case "SKIP_EVENT": {
      ...
      const open = content.storylets.find((s) =>
        evalConditions(state, s.requirements),
      );
      if (open) {
      ...
      return {
        state: next,
        log: ["사무소에는 아무도 없었다. 불을 끄고 정산 서류를 꺼낸다."],
      };
    }

    case "CHOOSE_STORYLET": {
      assertPhase(state, "event", "CHOOSE_STORYLET");
      const storylet = content.storylets.find(
        (s) => s.id === action.storyletId,
      );
      ...
      if (choice.startsMultiday && state.world.multiday) {
        throw new Error(
          `다일 이벤트 중복 점유 불가: ${state.world.multiday.id} 진행 중`,
        );
      }
      const rng = mulberry32(state.world.seed);
      ...
      if (choice.startsMultiday) {
        ...
        next.world.multiday = {
          id: choice.startsMultiday.id,
          daysLeft: choice.startsMultiday.days,
        };
    ...
    case "CONFIRM_DEBRIEF": {
      ...
      return {
        state: next,
        log: ["주말이다. 이틀 동안, 하루에 두 가지를 할 수 있다."],
      };
    }
    ...
    case "CHOOSE_WEEKEND": {
      ...
      if (!weekend || weekend.slotsLeft <= 0)
        throw new Error("주말 슬롯이 없음");
      const def = content.weekendActivities.find(
