...
function makeState(overrides?: { memory?: number; weekRatings?: GameState['world']['weekRatings'] }): GameState {
  return {
...
const CONTENT: ContentBundle = {
  ...
  orderTemplates: [
    {
      ...
      body: [
        { if: [{ path: 'self.memory', gte: 1 }], paragraphs: ['이제 이 문구가 무엇의 다른 이름인지 안다.'] },
  ...
  storylets: [
    { id: 'EV-001', requirements: [], body: [{ paragraphs: ['그가 대답했다.'] }], choices: [] },
  ],
};

describe('renderArchiveEntry — 카드', () => {
  const entry = { kind: 'order', day: 1, templateId: 'WO-R1', zone: 'd5' } as const;
  it('기억 0: 액면 그대로 (기준선)', () => {
  ...
  it('과거 문서도 현재 기억으로 렌더링한다 — day 1의 항목이 기억 1의 문장으로 열린다', () => {
    expect(renderArchiveEntry(entry, makeState({ memory: 1 }), CONTENT)).toEqual({
      title: '간헐 명멸 — 그 문구',
      paragraphs: ['이제 이 문구가 무엇의 다른 이름인지 안다.'],
    });
  });
  it('폐기된 템플릿 참조는 null — 목록에서 조용히 빠진다', () => {
    expect(renderArchiveEntry({ kind: 'order', day: 1, templateId: 'WO-GONE', zone: 'd5' }, makeState(), CONTENT)).toBeNull();
  });
});

describe('renderArchiveEntry — 면담·신고·통지', () => {
  it('스토리렛: 면담록 제목 + 본문', () => {
    expect(renderArchiveEntry({ kind: 'storylet', day: 2, id: 'EV-001' }, makeState(), CONTENT)).toEqual({
      title: '면담록 EV-001',
      paragraphs: ['그가 대답했다.'],
    });
  });
  it('조우: 신고서 제목 + intro', () => {
    expect(renderArchiveEntry({ kind: 'encounter', day: 3, id: 'ENC-001', zone: 'd5' }, makeState(), CONTENT)).toEqual({
      title: '설비 이상 확인 요청',
  ...
  it('등급이 없는 주의 통지서는 null', () => {
    expect(renderArchiveEntry({ kind: 'notice', day: 5, week: 2 }, makeState(), CONTENT)).toBeNull();
  });
});
