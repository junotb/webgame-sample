...
export function OnestrokeGame({ session, onFinish }: MinigameProps) {
  ...
  // 판은 설계 좌표(44px 칸) 그대로 그리고, 모자란 화면에는 통째로 축소해 앉힌다 —
  // 넘치면 flex 중앙정렬이 위(지시서 제목)와 아래(안내문)를 함께 침범한다 (whack과 같은 방식)
  const boardW = board.cols * (CELL + GAP) - GAP;
  const boardH = board.rows * (CELL + GAP) - GAP;
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setScale(Math.min(1, rect.width / boardW, rect.height / boardH));
    };
    update();
  ...
  return (
    // 세로 예산이 가장 빠듯한 판이다 — 지시·시계·안내문을 왼쪽 열로 보내고,
    // 판이 남은 높이 전부를 쓴다 (위아래로 쌓으면 기준 뷰포트에서 반드시 넘친다)
    <div className="minigame" data-minigame="onestroke" style={{ flexDirection: 'row', alignItems: 'center', gap: 26 }}>
      <div
        style={{
          display: 'flex',
      ...
      >
        <header className="minigame-head" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
          <span>
            구역 순찰 — 막힌 곳을 피해 모든 칸을 지나 끝 지점까지 ({trail.length}/{total})
          </span>
          <span className="minigame-clock">{Math.ceil(remaining / 1000)}</span>
        </header>
        <p className="minigame-note" style={{ margin: 0 }}>
