/**
 * 완곡어 재렌더 (세션 ⑤ — implementation-plan §4) — `(문서 항목, 현재 상태) → 문장`.
 *
 * 재열람 서류함의 계약이 이 함수 하나로 선다: 본문은 저장돼 있지 않고,
 * 열 때마다 **현재** 상태(기억·기술·노후도)로 변형이 다시 선택된다.
 * "그때 값으로 렌더링"은 금지 — entry의 day는 처음 겪은 날의 기록일 뿐,
 * 선택에 관여하지 않는다 (테스트로 고정, implementation-plan §5 구현 제약).
 *
 * 특수 카드 보류로 기억이 0에 고정된 지금은 언제나 같은 문장이 나온다.
 * 그 "똑같음"이 기준선이다 — 보류 해제 후 이 함수의 출력이 달라지는 것으로
 * 판정 목표 3(알아채는가)을 측정한다.
 */
