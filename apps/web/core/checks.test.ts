...
import { clamp, broadProbability, narrowProbability, checkProbability, mulberry32, rollCheck } from './checks';
...
describe('rollCheck — roll < p 성공, 같은 시드는 같은 결과', () => {
  ...
  it('성공 판정은 roll < p 와 일치', () => {
    const check = { kind: 'narrow', skill: 'flowsense', difficulty: 3 } as const; // p = 0.3
