...
export const QUESTS: QuestDef[] = [
  /* ---- 메인 체인 (자동 수주) ---- */
  {
    id: "m1", kind: "main", name: "숲의 이변",
    desc: "황혼의 숲 어귀에 고블린이 들끓는다. 길드가 정찰 겸 소탕을 의뢰했다.",
    objectives: [{ id: "kill_goblin", type: "kill", target: "goblin", count: 3, desc: "고블린 처치" }],
    rewards: { gold: 80, exp: 30 },
  },
  {
    id: "m2", kind: "main", name: "정예의 그림자",
    desc: "중앙 홀 길목을 오크 워로드가 막아섰다. 놈을 쓰러뜨려 길을 열어라.",
    requires: { quests: ["m1"] },
    objectives: [{ id: "clear_orc", type: "clear", target: "orc", count: 1, desc: "오크 워로드 토벌" }],
    rewards: { gold: 250, exp: 120, items: [{ id: "potion", n: 2 }] },
  },
  {
    id: "m3", kind: "main", name: "숲의 군주",
    desc: "숲 깊은 곳, 군주 그림바크가 잠에서 깨어났다. 왕국 최후의 의뢰다.",
    requires: { quests: ["m2"] },
  ...
  /* ---- 서브 (선택 수주) ---- */
  {
    ...
    desc: "숲 최북단 막다른 길에 무언가 숨겨져 있다는 소문. 탐색(Seek)이 필요할 것이다.",
    requires: { quests: ["m1"] },
    giver: "lokan",
  ...
  {
    id: "s2", kind: "side", name: "백골 소탕",
    desc: "심부의 스켈레톤이 순례길을 위협한다. 넷이면 충분히 조용해질 것이다.",
    requires: { level: 2 },
    giver: "martha",
    objectives: [{ id: "kill_skel", type: "kill", target: "skeleton", count: 4, desc: "스켈레톤 처치" }],
  ...
  /* ---- 반복 (재생성 몬스터만 — 보고 후 재수주 가능) ---- */
  {
    ...
    objectives: [{ id: "kill_slime", type: "kill", target: "slime", count: 5, desc: "점액 괴물 처치" }],
    rewards: { gold: 60, exp: 25 },
  },
  {
    id: "r2", kind: "repeat", name: "늑대 사냥",
    desc: "다이어울프 무리가 사냥꾼을 덮친다. 가죽은 길드가 사들인다.",
    requires: { level: 3 },
    objectives: [{ id: "kill_wolf", type: "kill", target: "wolf", count: 4, desc: "다이어울프 처치" }],
